...
export default function DashEvents() {
     ...
     console.log(currentUser._id)
      try {
        const res = await fetch(
          `/api/events/getadminevents?searchTerm=${searchTerm}`, 
          {
            method: "GET",
            credentials: "include",
            headers: {
  ...
  try {
    const res = await fetch(
      `/api/events/deleteevent/${productIdToDelete}/${currentUser._id}`,
      {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
  ...
  const handleunavailable = async (productId) => {
    try {
      const res = await fetch(`/api/events/unavailable/${productId}/${currentUser._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleAvailable = async (productId) => {
    try {
      const res = await fetch(`/api/events/available/${productId}/${currentUser._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    <div className='table-auto overflow-x-scroll md:mx-auto p-3 scrollbar scrollbar-track-slate-100 scrollbar-thumb-slate-300 dark:scrollbar-track-slate-700 dark:scrollbar-thumb-slate-500'>
      <div className='flex justify-between'>
        <input
          ...
          className="px-3 py-2 w-150 border border-pink-300 rounded-md focus:outline-none focus:border-pink-500 mr-2 h-10 dark:bg-slate-800 placeholder-gray-500"
        />
        <div></div>
        <Button
      ...
      <div className='flex-wrap flex gap-4 justify-center p-3'>
        <div className='flex flex-col p-3 dark:bg-slate-800 gap-4 md:w-72 w-full rounded-md shadow-md'>
          <div className='flex justify-between'>
            <div className=''>
              <h3 className='text-gray-500 text-md uppercase'>Total Events Created</h3>
      ...
      {currentUser.isTeam && userProduct.length > 0 ? (
        <>
            ...
            {userProduct.map((product) => (
              <Table.Body className='divide-y' key={product._id}>
                <Table.Row className='bg-white dark:border-gray-700 dark:bg-gray-800'>
                  ...
                  <Table.Cell>
                    <Link className='text-teal-500 hover:underline' to={`/update-event/${product._id}`}>
                      <span>Edit</span>
                    </Link>
                  </Table.Cell>
                </Table.Row>
      ...
      <Modal show={showModel} onClose={() => setShowModel(false)} popup size='md'>
        ...
        <Modal.Body>
          <div className="text-center">
            ...
            <h3 className="mb-5 text-lg text-gray-500 dark:text-gray-200">Are you sure you want to Delete this Event</h3>
          </div>
