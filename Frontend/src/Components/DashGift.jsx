...
export default function DashGift() {
     ...
     console.log(currentUser._id)
      try {
        const res = await fetch(
          `/api/gift/getadmingifts?searchTerm=${searchTerm}`, 
          {
            method: "GET",
            credentials: "include",
            headers: {
  ...
  try {
    const res = await fetch(
      `/api/gift/deletegift/${productIdToDelete}/${currentUser._id}`,
      {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
  ...
  const handleAssignFeature = async (productId) => {
    try {
      const res = await fetch(`/api/cakes/featuregift/${productId}/${currentUser._id}`, {
        method: 'PUT',
  ...
  const handleReassignFeature = async (productId) => {
    try {
      const res = await fetch(`/api/cakes/unfeaturegift/${productId}/${currentUser._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleunavailable = async (productId) => {
    try {
      const res = await fetch(`/api/gift/unavailable/${productId}/${currentUser._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleAvailable = async (productId) => {
    try {
      const res = await fetch(`/api/gift/available/${productId}/${currentUser._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    <div className='table-auto overflow-x-scroll md:mx-auto p-3 scrollbar scrollbar-track-slate-100 scrollbar-thumb-slate-300 dark:scrollbar-track-slate-700 dark:scrollbar-thumb-slate-500'>
      ...
      <div className='flex-wrap flex gap-4 justify-center p-3'>
        <div className='flex flex-col p-3 dark:bg-slate-800 gap-4 md:w-72 w-full rounded-md shadow-md'>
          <div className='flex justify-between'>
            <div className=''>
              <h3 className='text-gray-500 text-md uppercase'>Total Gift Made</h3>
        ...
        <>
          <Table hoverable className="shadow-md">
            <Table.Head>
              ...
              <Table.HeadCell>Gift Image</Table.HeadCell>
              <Table.HeadCell>Gift Title</Table.HeadCell>
            ...
            {userProduct.map((product) => (
              <Table.Body className='divide-y' key={product._id}>
                <Table.Row className='bg-white dark:border-gray-700 dark:bg-gray-800'>
                  ...
                  <Table.Cell>
                    <Link className='text-teal-500 hover:underline' to={`/update-gift/${product._id}`}>
                      <span>Edit</span>
                    </Link>
                  </Table.Cell>
                </Table.Row>
