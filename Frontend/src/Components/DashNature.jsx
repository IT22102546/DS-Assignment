...
export default function DashNature() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      if (!currentUser?._id) return; // Ensure user is logged in
     console.log(currentUser._id)
      try {
        const res = await fetch(
          `/api/nature/getadminnature?searchTerm=${searchTerm}`, 
          {
            method: "GET",
            credentials: "include",
            headers: {
              "Content-Type": "application/json",
              "userid": currentUser._id, // Send userId in headers
            },
          }
        );
    ...
    fetchProducts();
  }, [searchTerm, currentUser]); // Ensure it runs when the user changes
  
  const handleDeleteProduct = async () => {
    setShowModel(false);
    try {
      const res = await fetch(`/api/nature/deletenature/${productIdToDelete}/${currentUser._id}`, {
        method: 'DELETE',
        credentials: 'include'
      });
      const data = await res.json();
      if (!res.ok) {
        ...
        setUserProduct((prev) => prev.filter((product) => product._id !== productIdToDelete));
      }
    } catch (error) {
  ...
  const handleunavailable = async (productId) => {
    try {
      const res = await fetch(`/api/nature/unavailable/${productId}/${currentUser._id}`, {
        method: 'PUT',
      ...
      if (res.ok) {
        setUserProduct((prev) =>
          prev.map((product) => product._id === productId ? { ...product, isAvailable: false } : product)
        );
      } else {
        console.log(data.message);
      }
  ...
  const handleAvailable = async (productId) => {
    try {
      const res = await fetch(`/api/nature/available/${productId}/${currentUser._id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include'
      });
      ...
      if (res.ok) {
        setUserProduct((prev) =>
          prev.map((product) => product._id === productId ? { ...product, isAvailable: true } : product)
        );
      } else {
        console.log(data.message);
      }
  ...
  const generatePDFReport = () => {
    ...
    html2pdf().from(content).set({ margin: 1, filename: 'sweet_report.pdf' }).save();
  };
  ...
  return (
    ...
    <div className='flex justify-between'>
      <input
        type="text"
        placeholder="Search nature products.."
        value={searchTerm}
    ...
    <div className='flex-wrap flex gap-4 justify-center p-3'>
      <div className='flex flex-col p-3 dark:bg-slate-800 gap-4 md:w-72 w-full rounded-md shadow-md'>
        <div className='flex justify-between'>
          <div className=''>
            <h3 className='text-gray-500 text-md uppercase'>Total Nature Products</h3>
    ...
    {currentUser.isAdmin && userProduct.length > 0 ? (
      <>
        <Table hoverable className="shadow-md">
          <Table.Head>
            ...
            <Table.HeadCell>Product Weight</Table.HeadCell>
          ...
          {userProduct.map((product) => (
            <Table.Body className='divide-y' key={product._id}>
              <Table.Row className='bg-white dark:border-gray-700 dark:bg-gray-800'>
                ...
                <Table.Cell>
                  <Link className='font-medium text-gray-900 dark:text-white' to={`/nature/${product.slug}`}>
                    {product.title}
                  </Link>
                </Table.Cell>
                ...
                <Table.Cell>
                  <Link className='text-teal-500 hover:underline' to={`/update-nature/${product._id}`}>
                    <span>Edit</span>
                  </Link>
                </Table.Cell>
              </Table.Row>
      ...
      <p>You have no Nature Products to show</p>
    )}
