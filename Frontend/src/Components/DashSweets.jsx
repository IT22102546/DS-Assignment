...
export default function DashSweets() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await fetch(`/api/sweets/getadminsweet?searchTerm=${searchTerm}`);
  ...
  const handleDeleteProduct = async () => {
    ...
    try {
      const res = await fetch(
        `/api/sweets/deletesweet/${productIdToDelete}/${currentUser._id}`,
        {
          method: 'DELETE',
        }
      );
  ...
  const handleAssignFeature = async (productId) => {
    try {
      const res = await fetch(`/api/sweets/featuresweet/${productId}/${currentUser._id}`, {
        method: 'PUT',
  ...
  const handleReassignFeature = async (productId) => {
    try {
      const res = await fetch(`/api/sweets/unfeaturesweet/${productId}/${currentUser._id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleunavailable = async (productId) => {
    try {
      const res = await fetch(`/api/sweets/unavailable/${productId}/${currentUser._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleAvailable = async (productId) => {
    try {
      const res = await fetch(`/api/sweets/available/${productId}/${currentUser._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    <div className='table-auto overflow-x-scroll md:mx-auto p-3 scrollbar scrollbar-track-slate-100 scrollbar-thumb-slate-300 dark:scrollbar-track-slate-700 dark:scrollbar-thumb-slate-500'>
      ...
      <div className='flex-wrap flex gap-4 justify-center p-3'>
        <div className='flex flex-col p-3 dark:bg-slate-800 gap-4 md:w-72 w-full rounded-md shadow-md'>
          <div className='flex justify-between'>
            <div className=''>
              <h3 className='text-gray-500 text-md uppercase'>Total Sweets</h3>
      ...
      {currentUser.isAdmin && userProduct.length > 0 ? (
        <>
          <Table hoverable className="shadow-md">
            <Table.Head>
              ...
              <Table.HeadCell>Sweet Image</Table.HeadCell>
              <Table.HeadCell>Sweet Title</Table.HeadCell>
            ...
            {userProduct.map((product) => (
              <Table.Body className='divide-y' key={product._id}>
                <Table.Row className='bg-white dark:border-gray-700 dark:bg-gray-800'>
                  ...
                  <Table.Cell>
                    <Link className='font-medium text-gray-900 dark:text-white' to={`/sweet/${product.slug}`}>
                      {product.title}
                    </Link>
                  </Table.Cell>
                  ...
                  <Table.Cell>
                    <Link className='text-teal-500 hover:underline' to={`/update-sweet/${product._id}`}>
                      <span>Edit</span>
                    </Link>
                  </Table.Cell>
                </Table.Row>
        ...
        <p>You have no Sweets to show</p>
      )}
      <Modal show={showModel} onClose={() => setShowModel(false)} popup size='md'>
        ...
        <Modal.Body>
          <div className="text-center">
            ...
            <h3 className="mb-5 text-lg text-gray-500 dark:text-gray-200">Are you sure you want to Delete this Sweet</h3>
          </div>
