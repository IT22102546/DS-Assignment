...
import { HiUser, HiMenu, HiSearch, HiX, HiOutlineShoppingCart, HiHome, HiCollection, HiMusicNote, HiBookOpen, HiUserGroup, HiCake, HiOutlineTable, HiGift } from "react-icons/hi";
...
export default function Header() {
      ...
      <div className="container mx-auto flex items-center justify-between px-4">
        ...
        <div className="relative flex-grow max-w-2xl hidden md:block">
          <input
            type="text"
            placeholder="Search..."
            className="w-full px-4 py-2 rounded-lg border border-gray-300 focus:outline-none bg-gradient-to-b from-[rgba(254,129,128,0.3)] via-[rgba(255,255,255,0.3)] to-[rgba(254,143,142,0.3)] text-gray-800 placeholder-gray-700"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && handleSearch()}
          />
          <HiSearch className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-500 cursor-pointer" onClick={handleSearch} />
        </div>
  ...
  { name: "Sweets", path: "/sweets", icon: <GiCandyCanes size={20} /> },

  { name: "Nature", path: "/nature", icon: <FaLeaf size={20} /> },
  { name: "Design", path: "/design", icon: <HiOutlineTable size={20} /> },
  { name: "AboutUs", path: "/aboutus", icon: <HiUserGroup size={20} /> },
  { name: "Booking", path: "/booking", icon: <HiBookOpen size={20} /> },
  { name: "Gift Boxes", path: "/gift", icon: <HiGift size={20} /> },
  { name: "Surprise Teams", path: "/Suprise", icon: <GiPartyPopper size={20} /> },
].map((item, index) => (
  <motion.button
    key={index}
 ...
 <div className="flex items-center space-x-4">
        ...
        <button className="md:hidden ml-auto" onClick={() => setMenuOpen(!menuOpen)}>
          {menuOpen ? <HiX className="text-white text-2xl" /> : <HiMenu className="text-white text-2xl" />}
        </button>
      </div>
        ...
        {menuOpen && (
          ...
          >
            <input
              type="text"
              placeholder="Search..."
              className="w-full px-4 py-2 rounded-full border border-gray-300 focus:outline-none bg-gradient-to-b from-[rgba(254,129,128,0.3)] via-[rgba(255,255,255,0.3)] to-[rgba(254,143,142,0.3)] text-gray-800 placeholder-gray-700"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && handleSearch()}
            />
            <nav className="flex flex-col space-y-2 mt-2">
              ...
              <Dropdown label="Products" inline>
                <DropdownItem onClick={() => navigate("/cakes")}>Cakes</DropdownItem>
                <DropdownItem onClick={() => navigate("/sweets")}>Sweets</DropdownItem>
                <DropdownItem onClick={() => navigate("/nature")}>Nature Products</DropdownItem>
              </Dropdown>
              ...
              <NavLink to="/suprise" className={({ isActive }) => isActive ? "font-semibold text-black" : "font-semibold"}>Surprise Team</NavLink>
              <NavLink to="/gift" className={({ isActive }) => isActive ? "font-semibold text-black" : "font-semibold"}>Gift Boxes</NavLink>
            </nav>
          </motion.div>
        )}
      </AnimatePresence>
