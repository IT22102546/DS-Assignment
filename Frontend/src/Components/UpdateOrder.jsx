...
export default function UpdateOrder() {
  ...
  const apiUrl2 = import.meta.env.VITE_Inventory_API_UR;
  ...
  useEffect(() => {
    const fetchOrderAndDeliveryPersons = async () => {
      try {
        ...
        // Fetch order details
        const orderRes = await fetch(`${apiUrl}/api/order/getorder/${orderId}`);
        ...
        if (orderRes.ok) {
          ...
          if (orderData.status === "handover" && orderData.deliveryPerson) {
            setSelectedDeliveryPerson(
              orderData.deliveryPerson._id || orderData.deliveryPerson
            );
          }
        } else {
        ...
        // const deliveryRes = await fetch(`${apiUrl2}/api/inventory/getriders`);
        ...
        //     !ridersArray.some((r) => r._id === orderData.deliveryPerson._id)
        //   ) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/api/order/update-status/${orderId}`, {
        method: "PUT",
        headers: {
      ...
      {order && (
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            ...
            <TextInput type="text" value={order.orderId} readOnly />
          </div>
