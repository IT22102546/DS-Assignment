...
export default function AddSweets() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch('/api/sweets/create', { 
        method: 'POST',
        headers: {
  ...
  return (
    ...
    <div className="p-3 max-w-3xl mx-auto min-h-screen">
      <h1 className="text-center text-3xl my-7 font-semibold">Add Sweet</h1>
      <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
        ...
        {[0, 1, 2, 3].map((index) => (
          <div key={index} className='flex gap-4 items-center justify-between border-4 border-teal-500 border-dotted p-3'>
            ...
            <Button onClick={() => setMainImageIndex(index)} type='button' size='sm' className={`bg-slate-400 ${mainImageIndex === index ? 'bg-[#FE8180]' : ''}`}>
        ...
        <Button type='submit' className="max-w-md w-full bg-[#FE8180] hover:bg-[#e57373] mx-auto flex items-center justify-center">Add Sweet</Button>
