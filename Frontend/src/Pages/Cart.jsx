...
export default function Cart() {
  ...
  useEffect(() => {
    ...
    if (user && cart.cartItems.length > 0) {
      ...
      if (userCartItems.length > 0) {
        ...
        const allSameShop = userCartItems.every(item => item.storename === firstShop);
  ...
  return (
    <div className="bg-gradient-to-b from-[rgba(254,129,128,0.3)] via-[rgba(255,255,255,0.3)] to-[rgba(254,143,142,0.3)] py-10 px-6">
          ...
          <div className="relative">
            <ul className="relative flex w-full items-center justify-between space-x-2 sm:space-x-4">
              <li className="flex items-center space-x-3 text-left sm:space-x-4">
                <a className="flex h-6 w-6 items-center justify-center rounded-full bg-blue-200 text-xs font-semibold text-blue-700">
          ...
          <div className="w-full max-w-7xl px-4 md:px-5 lg-6 mx-auto font-cinzel text-black">
            ...
            {shopMismatch && (
              <div className="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-6" role="alert">
                <p className="font-bold font-cinzel">Shop Mismatch Detected</p>
                <p className="font-cinzel">You have items from different shops in your cart. Please select items from the same shop to continue. Current shop is "{currentShop}".</p>
                <button 
                  onClick={handleClearCart}
                  className="mt-2 text-red-700 underline font-cinzel"
                >
            ...
            <div className="flex flex-col lg:flex-row gap-12 lg:items-start">
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              ...
                              <Button 
                                ...
                                className="mt-2 text-white bg-[#FE8180]"
                              >
              ...
              <div className="basis-4/12">
                <div className="border border-gray-200 bg-white shadow-sm rounded-xl py-8 px-6">
                  ...
                  <button onClick={handleClearCart} className="inline-block w-full bg-[#FE8180] text-center text-white py-3 px-4 rounded-md mt-6 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#FE8180]">
                    Clear Cart
                  </button>
                  <Link to={shopMismatch ? "#" : "/order-summary"}>
                    <button 
                      disabled={shopMismatch}
                      className={`inline-block w-full text-center text-white py-3 px-4 rounded-md mt-6 focus:outline-none focus:ring-2 focus:ring-offset-2 ${shopMismatch ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#FE8180] focus:ring-[#FE8180]'}`}
                    >
                      {shopMismatch ? 'Select from same shop to continue' : 'Proceed to Order Summary'}
                    </button>
                  </Link>
                </div>
              </div>
