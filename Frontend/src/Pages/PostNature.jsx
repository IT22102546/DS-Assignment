...
export default function PostNature() {
  const { natureSlug } = useParams();
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        setLoading(true);
        const res = await fetch(`/api/nature/getnature?slug=${natureSlug}`);
        ...
        const similarRes = await fetch(`/api/nature/category?category=${data.products[0].category}`);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {similarProducts.length > 0 ? (
            similarProducts.map((similarProduct, index) => (
              <Link key={index} to={`/nature/${similarProduct.slug}`}>
