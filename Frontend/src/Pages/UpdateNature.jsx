...
export default function UpdateNature() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await fetch(`/api/nature/getnature?productId=${productId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        `/api/nature/updatenature/${formData._id}/${currentUser._id}`,
        {
          method: "PUT",
          headers: {
