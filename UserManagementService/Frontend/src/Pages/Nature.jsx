...
export default function Nature() {
  ...
  const fetchProducts = async () => {
    try {
      const res = await fetch(
        `/api/nature/getnature?searchTerm=${searchTerm}&priceRange=${selectedPriceRange}`
      );
  ...
  const handleSearchSubmit = () => {
    ...
    navigate(`/nature?searchTerm=${encodeURIComponent(searchTerm)}&priceRange=${selectedPriceRange}`);
  };
  ...
  const handlePriceRangeChange = (e) => {
    ...
    navigate(`/nature?searchTerm=${searchTerm}&priceRange=${e.target.value}`);
  };
        ...
        {products.map((product) => (
          ...
          >
            ...
            <h3 className="text-lg font-semibold text-center mb-2 hover:text-pink-600">
              <Link to={`/nature/${product.slug}`}>{product.title}</Link>
            </h3>
