...
export default function PostGift() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        setLoading(true);
        const res = await fetch(`/api/gift/getgifts?slug=${giftSlug}`);
        ...
        const similarRes = await fetch(`/api/gift/category?category=${data.products[0].category}`);
    ...
    >
      ...
      <div className="flex flex-col lg:flex-row gap-8 mt-8">
        ...
        <div className="lg:w-1/2">
          ...
          >
            {/* Main Image Section */}
            <div className="mb-6 rounded-xl overflow-hidden shadow-lg p-2 h-96">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {similarProducts.length > 0 ? (
            similarProducts.map((similarProduct, index) => (
              <Link key={index} to={`/gift/${similarProduct.slug}`}>
